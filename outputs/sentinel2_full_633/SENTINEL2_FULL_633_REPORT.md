...
> - **OAuth2 Configuration Status:** `CONFIGURED (CDSE_CLIENT_ID / CDSE_CLIENT_SECRET present)`
...
| **Events with Real Pre + Post Imagery** | **42** | Both observations retrieved and change features calculated |
| **Events with Real Pre Only** | **77** | Pre observation retrieved; post unavailable |
| **Events with Real Post Only** | **53** | Post observation retrieved; pre unavailable |
| **Events with Neither Observation** | **461** | Neither pre nor post observation could be retrieved |
| **Total Successful Real Observations** | **214** | Count of individual S2 L2A observations processed |
| **Total Real Pixels Processed** | **535,000** | (2,500 pixels/observation) |
| **Sentinel-2 Real Data Coverage** | **16.90%** | Ratio of retrieved to possible observation slots (633 x 2 = 1,266) |
| **Total Execution Time** | **3037.05 seconds** | End-to-end processing duration |
...
| :--- | :--- | :--- | :--- |
| `REAL_CDSE_SUCCESS` | 42 | 6.6% | Both pre and post real GeoTIFF rasters successfully processed |
| `PARTIAL_PRE_ONLY` | 77 | 12.2% | Pre observation processed; post unavailable |
| `PARTIAL_POST_ONLY` | 53 | 8.4% | Post observation processed; pre unavailable |
| `AUTH_REQUIRED` | 0 | 0.0% | CDSE credentials not provided; pipeline halted cleanly |
| `CLOUD_REJECTED` | 150 | 23.7% | Real imagery returned but SCL valid surface coverage < 70% |
| `MISSING_PRODUCT` | 70 | 11.1% | Neither pre nor post observation found in catalogue search |
| `PROCESSING_API_ERROR` | 241 | 38.1% | CDSE HTTP timeout or gateway error |
